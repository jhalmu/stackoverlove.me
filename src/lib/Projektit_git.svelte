		...
		<div class="card-body content-center">
			...
			<div class="flex justify-center flex-wrap h-20">
				{#each languages.nodes as nuudeli}
					<span class="badge badge-accent min-w-fit p-4 m-1 text-cyan-800" title="tag">{nuudeli.name}</span>
				{/each}
			</div>
			<div class="card-actions flex-wrap justify-center content-end mt-5 h-40 space-x-5">
				...
				<a href={url} target="_blank" >
					<label for="githublink" tabindex="0" title="Read {name}'s source in GitHub" aria-label="Read {name}'s source in GitHub">
